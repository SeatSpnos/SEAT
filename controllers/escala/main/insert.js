...
const scaleModel = require('models/escala').main;
...
function one (req, res, next) {
  scaleModel.insert.one(req.body, function (err, results) {
...
function bulk (req, res, next) {
  let values = {
    ...
    value_FK_ID: req.body.value_FK_ID
  };
  let toSend = [];
  ...
  for (
    ...
    currentDate.add(1, 'd')) {
    let tempArry = [currentDate.format('YYYY-MM-DD'), values.user_FK_ID, values.value_FK_ID];
    toSend.push(tempArry);
  }
  scaleModel.insert.bulk([toSend], (err, results) => {
    if (err) return res.status(500).json(err);
    res.status(200).json(results);
  });
}
